...
sub add_role {
   ...
   $self->schema->resultset('RolePermission')->delete({ role_id => $role->id });
   if (@perms) {
...
sub add_user {
   ...
   $self->schema->resultset('UserRole')->delete({ user_id => $user->id });
   if (@roles) {
...
sub add_screen {
   ...
   $self->schema->resultset('PermissionScreen')->delete({ screen_id => $screen->id });
   if (@perms) {
